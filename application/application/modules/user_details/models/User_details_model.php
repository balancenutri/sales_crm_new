   ...
   public function get_user_details($status=false,$email=false){
        ...
        if($status=='client'){
            ...
            $sql = "SELECT
                        ...
                        lm.sub_status,
                        lm.sub_status_note,lm.lifestyle,CONCAT(lm.medical_issue,',', lm.clinical_condition) AS medical_issue,lm.diet_history,lm.age_group,lm.target_oriented,lm.motivation_level,
                        CONCAT(lm.fname, ' ', lm.lname) AS name,
                        lm.email as email_id,
                        lm.ip_address as ip_address,
                        lm.phone ,
                        lm.country, lm.state,lm.city,
    ...
    ELSE '' 
END as msg FROM `lead_action` where (reminder_dt > '".date('Y-m-d H:i:s')."' OR fu_other_date > '".date('Y-m-d H:i:s')."') and email = '$email' limit 1
                    ) AS next_action,
                    (
                        SELECT
                            assign_to
